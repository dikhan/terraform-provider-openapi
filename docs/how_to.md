...
if input is not given, that's the purpose of ```Optional computed``` properties.
  ...
  two use cases under the optional-computed scenario:
     - Optional computed with default: The default value is known at runtime. In this case, the service provider should document 
     ...
     - Optional computed: The default value is NOT known at runtime: In this case, the value is probably autogenerated by 
     the API and therefore the value is not known at runtime. Hence, the user should attach to the property the 
     *‘x-terraform-optional-computed’* attribute so the OpenAPI Terraform provider will understand the behaviour expected. 
...
in responses but not in requests, and the value is automatically assigned by the API. See example below **computed**
     ...
     computed value is known at runtime and enables the service provider to document the behaviour of the property and be 
...
definitions:
  resource:
    ...
    required:
      - mandatory_property
    properties:
      id: # the value of this computed property is not known at runtime (e,g: uuid, etc)
        type: string
        readOnly: true
      ...
      computed: # computed property that the value is NOT know at runtime, and the API will compute one and return it in the response
        type: "string"
        readOnly: true 
