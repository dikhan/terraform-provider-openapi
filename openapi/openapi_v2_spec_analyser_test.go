...
func Test_mergeRequestAndResponseSchemas(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "request properties are readOnly, this returns an error since the merge method expects the request schema to only contain input properties",
			requestSchema: &spec.Schema{
				SchemaProps: spec.SchemaProps{
					Properties: map[string]spec.Schema{
						"some_property": {
			...
			expectedMergedSchema: nil,
			expectedError:        "resource's request schema property 'some_property' can not be readOnly as request properties are considered inputs",
		},
		{
			name: "response property matching the request one is not readOnly, this returns an error since the merge method expects the matching request schema properties in the response to be readOnly since they will be returned by the API",
			requestSchema: &spec.Schema{
				SchemaProps: spec.SchemaProps{
					Properties: map[string]spec.Schema{
						"some_property": {
			...
			expectedMergedSchema: nil,
			expectedError:        "resource's response schema property 'some_property' must be readOnly as response properties are considered computed (returned by the API)",
		},
		{
