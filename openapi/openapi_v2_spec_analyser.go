...
// - the request schema must contain only properties that are either required or optional, not readOnly
// - the response schema must contain only properties that are readOnly
...
func (specAnalyser *specV2Analyser) mergeRequestAndResponseSchemas(requestSchema *spec.Schema, responseSchema *spec.Schema) (*spec.Schema, error) {
	...
	// Ensure only the request's required properties are kept for the merged schema
	...
	for requestSchemaPropName, requestSchemaProp := range requestSchemaProps {
		...
		// The check below is put in place to ensure compliance with OpenAPI spec 2.0:
		// https://swagger.io/specification/v2/#fixed-fields-13 (readOnly section)
		...
		if requestSchemaProp.ReadOnly {
			return nil, fmt.Errorf("resource's request schema property '%s' can not be readOnly as request properties are considered inputs", requestSchemaPropName)
		}

		if !responseSchemaProps[requestSchemaPropName].ReadOnly {
			return nil, fmt.Errorf("resource's response schema property '%s' must be readOnly as response properties are considered computed (returned by the API)", requestSchemaPropName)
		}
